// Photo gallery — striped placeholders + lightbox
const galleryImages = [
      ...
      {open !== null && (
        <div onClick={() => setOpen(null)}
          ...
          <div onClick={(e) => e.stopPropagation()}
            ...
            <div className="placeholder" style={{ height: 500, marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, marginTop: 6, whiteSpace: 'pre-line' }}>
                  {galleryImages[open].label}
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <button className="btn btn-outline"
                  onClick={() => setOpen((open - 1 + galleryImages.length) % galleryImages.length)}>
                  ← Prev
                </button>
                <button className="btn btn-outline"
                  onClick={() => setOpen((open + 1) % galleryImages.length)}>
                  Next →
                </button>
